                ...
                void ProcessTime(String TimeName)
                {
                    ...
                    for (int i = 0; i < divs.Count; i++)
                    {
                        ...
                        for (int j = 0; j < rows1.Count; j++)
                        {
                            ...
                            if (columns1[1].InnerText.IndexOf('-') == -1)
                            {
                                ((verb as IDictionary<string, object>)[TimeName] as IDictionary<string, Dictionary<String, String>>)[LocalTimeName].Add(columns1[0].InnerText, columns1[1].InnerText);
                                Console.WriteLine(columns1[0].InnerText + "->" + columns1[1].InnerText);
                            }
                        }
